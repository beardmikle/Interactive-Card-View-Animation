...
//
//  Created by beardmikle on 28.01.2023.
//
...
class ViewController: UIViewController {
    ...
    func setupCard() {
        ...
        cardViewController.view.frame = CGRect(x: 0, y: self.view.frame.height - CGFloat(cardHandleAreaHeight), width: self.view.bounds.width, height: cardHeight)
    ...
    func animateTransitionIfNeeded(state: CardState, duration: TimeInterval) {
        if runningAnimations.isEmpty {
            let frameAnimator = UIViewPropertyAnimator(duration: duration, dampingRatio: 1) {
                ...
                case .collapsed:
                    self.cardViewController.view.frame.origin.y = self.view.frame.height - CGFloat(self.cardHandleAreaHeight)
                }
            }
